...
def lsp_initialize_params(
    ...
    # Advertise semanticTokens with ``requests.full.delta`` so servers
    # return token deltas (a small edit set keyed off a previous resultId)
    # instead of the whole token array on every refresh. The standard
    # LSP 3.17 legend + ``relative`` format mirror what vscode sends, so
    # full semantic tokens keep working for servers that don't do delta.
    var sem_caps = json_object()
    sem_caps.put(String("dynamicRegistration"), json_bool(False))
    var sem_requests = json_object()
    var sem_full = json_object()
    sem_full.put(String("delta"), json_bool(True))
    sem_requests.put(String("full"), sem_full^)
    sem_requests.put(String("range"), json_bool(False))
    sem_caps.put(String("requests"), sem_requests^)
    var sem_types = json_array()
    var std_types = List[String]()
    std_types.append(String("namespace")); std_types.append(String("type"))
    std_types.append(String("class")); std_types.append(String("enum"))
    std_types.append(String("interface")); std_types.append(String("struct"))
    std_types.append(String("typeParameter"))
    std_types.append(String("parameter")); std_types.append(String("variable"))
    std_types.append(String("property"))
    std_types.append(String("enumMember")); std_types.append(String("event"))
    std_types.append(String("function")); std_types.append(String("method"))
    std_types.append(String("macro")); std_types.append(String("keyword"))
    std_types.append(String("modifier")); std_types.append(String("comment"))
    std_types.append(String("string")); std_types.append(String("number"))
    std_types.append(String("regexp")); std_types.append(String("operator"))
    std_types.append(String("decorator"))
    for i in range(len(std_types)):
        sem_types.append(json_str(std_types[i]))
    sem_caps.put(String("tokenTypes"), sem_types^)
    var sem_mods = json_array()
    var std_mods = List[String]()
    std_mods.append(String("declaration")); std_mods.append(String("definition"))
    std_mods.append(String("readonly")); std_mods.append(String("static"))
    std_mods.append(String("deprecated")); std_mods.append(String("abstract"))
    std_mods.append(String("async")); std_mods.append(String("modification"))
    std_mods.append(String("documentation"))
    std_mods.append(String("defaultLibrary"))
    for i in range(len(std_mods)):
        sem_mods.append(json_str(std_mods[i]))
    sem_caps.put(String("tokenModifiers"), sem_mods^)
    var sem_formats = json_array()
    sem_formats.append(json_str(String("relative")))
    sem_caps.put(String("formats"), sem_formats^)
    text_doc_caps.put(String("semanticTokens"), sem_caps^)
