...
from .events import (
    Event, EVENT_KEY, EVENT_MOUSE, KEY_BACKSPACE, KEY_DELETE, KEY_DOWN,
    KEY_END, KEY_ENTER, KEY_HOME, KEY_LEFT, KEY_PAGEDOWN, KEY_PAGEUP,
    KEY_RIGHT, KEY_UP, MOD_ALT, MOD_CTRL, MOD_META, MOD_SHIFT,
    MOUSE_BUTTON_LEFT,
)
...
from .string_utils import (
    ...
    utf8_cell_of_byte, utf8_codepoint_size, utf8_step_forward,
...
# --- TextArea: the multi-line counterpart --------------------------------


def _split_lines_exact(text: String) -> List[String]:
    """Split on ``\\n`` keeping every empty line, including a trailing
    one. ``"a\\nb"`` -> ``["a", "b"]``; ``"a\\n"`` -> ``["a", ""]``;
    ``""`` -> ``[""]``.

    Deliberately *not* ``split_lines_no_trailing``: an editor has to
    round-trip exactly, and a trailing blank line the user typed is
    content, not noise.
    """
    var out = List[String]()
    var b = text.as_bytes()
...
struct TextArea(Copyable, Movable):
    """Multi-line editable text: the counterpart to ``TextField`` for
    inputs that genuinely need newlines (a commit message body).

    ``lines`` is the buffer, one entry per line, never empty (an empty
    area holds one empty line). ``row`` indexes ``lines`` and ``col`` is
    a *byte* offset within ``lines[row]``, always on a UTF-8 codepoint
    boundary — the same invariant ``TextField.cursor`` keeps.

    Column arithmetic goes through ``utf8_cell_of_byte`` /
    ``utf8_byte_of_cell`` rather than byte or codepoint counts, so a
    double-width glyph occupies two cells and the caret can't park
    inside one (see CLAUDE.md "Emoji are two cells").

    Deliberately narrower than ``Editor``: no selection, no undo, no
    syntax highlighting. It's an input widget, not a buffer view — if
    you need those, you want ``Editor``. What it does support is
    caret movement, insert / delete / split / join, click-to-position,
    multi-line paste, and scrolling in both axes.

    ``Enter`` inserts a newline, so it does **not** consume
    Cmd/Ctrl+Enter — that chord is left for the host to use as "submit"
    (which is exactly what the commit-message overlay does).
    """

    var lines: List[String]
    var row: Int
    var col: Int              # byte offset within lines[row]
    var scroll_row: Int
    var scroll_col: Int
    # Cell column the caret "wants" when moving vertically, so walking
    # down through a short line and back out doesn't collapse the caret
    # to the short line's end. Reset by any horizontal move or edit.
    var _goal_cell: Int
    var _has_goal: Bool
    # Viewport height in rows, recorded by ``paint`` so PageUp/PageDown
    # can move by a real page. Same trick ``TextField`` uses to learn
    # its width for horizontal scrolling.
    var _page: Int
    ...
    def __copyinit__(mut self, copy: Self):
        ...
        self.row = copy.row
        self.col = copy.col
        self.scroll_row = copy.scroll_row
        self.scroll_col = copy.scroll_col
        self._goal_cell = copy._goal_cell
        self._has_goal = copy._has_goal
        self._page = copy._page

    def copy(self) -> Self:
        var out = Self()
        out.lines = self.lines.copy()
        out.row = self.row
        out.col = self.col
        out.scroll_row = self.scroll_row
        out.scroll_col = self.scroll_col
        out._goal_cell = self._goal_cell
        out._has_goal = self._has_goal
    ...
    def set_text(mut self, text: String):
        """Replace the buffer and park the caret at the very end — the
        natural spot when prefilling an existing message for editing."""
        self.lines = _split_lines_exact(text)
        self.row = len(self.lines) - 1
        self.col = len(self.lines[self.row].as_bytes())
        self.scroll_row = 0
        self.scroll_col = 0
        self._has_goal = False

    def text(self) -> String:
        """The buffer joined with ``\\n``. Round-trips ``set_text``."""
        var out = String("")
    ...
    def is_empty(self) -> Bool:
        """True when the buffer holds nothing but whitespace — what a
        caller checks before accepting the content as a message."""
        return len(self.text().strip().as_bytes()) == 0

    # --- caret --------------------------------------------------------

    def _clamp_row(mut self):
        if self.row < 0:
            self.row = 0
    ...
    def cursor_cell(self) -> Int:
        """Caret position as a display column within its line."""
        return utf8_cell_of_byte(self.lines[self.row], self.col)

    def _set_col_from_cell(mut self, cell: Int):
        """Place the caret at display column ``cell`` on the current
        line, clamped to the line's end."""
        var line = self.lines[self.row]
        var width = display_columns(line)
        var target = cell
        if target > width:
            target = width
        if target < 0:
            target = 0
        self.col = utf8_byte_of_cell(line, target)

    def _move_vertical(mut self, delta: Int):
        if not self._has_goal:
            self._goal_cell = self.cursor_cell()
            self._has_goal = True
        var goal = self._goal_cell
        self.row += delta
        self._clamp_row()
        self._set_col_from_cell(goal)
        # Restore the goal: ``_set_col_from_cell`` may have clamped to a
        # shorter line, but the next vertical move should still aim for
        # the original column.
        self._goal_cell = goal
        self._has_goal = True

    def _move_left(mut self):
        self._has_goal = False
        if self.col > 0:
            self.col = prev_codepoint_start(self.lines[self.row], self.col)
        elif self.row > 0:
            self.row -= 1
            self.col = len(self.lines[self.row].as_bytes())

    def _move_right(mut self):
        self._has_goal = False
        var line = self.lines[self.row]
        if self.col < len(line.as_bytes()):
            self.col = utf8_step_forward(line, self.col)
    ...
    def insert(mut self, s: String) -> Bool:
        """Insert ``s`` at the caret, honoring embedded newlines (so a
        multi-line paste lands as multiple lines). Returns whether
        anything changed."""
        if len(s.as_bytes()) == 0:
            return False
        self._has_goal = False
        var chunks = _split_lines_exact(s)
        var line = self.lines[self.row]
        var lb = line.as_bytes()
        var head = String(StringSpan(unsafe_from_utf8=lb[0:self.col]))
        var tail = String(StringSpan(unsafe_from_utf8=lb[self.col:len(lb)]))
        if len(chunks) == 1:
            self.lines[self.row] = head + chunks[0] + tail
            self.col = len(head.as_bytes()) + len(chunks[0].as_bytes())
            return True
        # Multi-line: first chunk finishes the current line, last chunk
        # starts the tail, everything between becomes a fresh line.
        self.lines[self.row] = head + chunks[0]
        var insert_at = self.row + 1
        for i in range(1, len(chunks) - 1):
            self.lines.insert(insert_at, chunks[i])
            insert_at += 1
        var last = chunks[len(chunks) - 1]
        self.lines.insert(insert_at, last + tail)
        self.row = insert_at
        self.col = len(last.as_bytes())
        return True

    def _split_line(mut self):
        self._has_goal = False
        var line = self.lines[self.row]
        var lb = line.as_bytes()
        var head = String(StringSpan(unsafe_from_utf8=lb[0:self.col]))
    ...
    def _backspace(mut self) -> Bool:
        self._has_goal = False
        if self.col > 0:
            var line = self.lines[self.row]
            var start = prev_codepoint_start(line, self.col)
            self.lines[self.row] = _splice(line, start, self.col, String(""))
            self.col = start
            return True
        if self.row == 0:
            return False
        # Join with the previous line; the caret lands at the seam.
        var prev = self.lines[self.row - 1]
        var seam = len(prev.as_bytes())
        self.lines[self.row - 1] = prev + self.lines[self.row]
        _ = self.lines.pop(self.row)
        self.row -= 1
        self.col = seam
        return True

    def _delete_forward(mut self) -> Bool:
        self._has_goal = False
        var line = self.lines[self.row]
        var lb = len(line.as_bytes())
        if self.col < lb:
            var nxt = utf8_step_forward(line, self.col)
            self.lines[self.row] = _splice(line, self.col, nxt, String(""))
            return True
        if self.row + 1 >= len(self.lines):
            return False
        # Pull the next line up onto this one.
        self.lines[self.row] = line + self.lines[self.row + 1]
        _ = self.lines.pop(self.row + 1)
        return True

    # --- input --------------------------------------------------------

    def handle_key(mut self, event: Event) -> TextFieldKeyResult:
        """Apply ``event``. Returns ``(consumed, changed)`` with the same
        contract as ``TextField.handle_key``.

        Cmd/Ctrl+Enter is explicitly *not* consumed so the host can bind
        it to submit; plain Enter inserts a newline.
        """
        if event.kind != EVENT_KEY:
            return TextFieldKeyResult(False, False)
        var k = event.key
        var chord = (event.mods & (MOD_CTRL | MOD_META)) != 0
        # Leave the submit chord to the host.
        if k == KEY_ENTER and chord:
            return TextFieldKeyResult(False, False)
        if k == KEY_ENTER:
            self._split_line()
            return TextFieldKeyResult(True, True)
        if k == KEY_BACKSPACE:
            return TextFieldKeyResult(True, self._backspace())
        if k == KEY_DELETE:
            return TextFieldKeyResult(True, self._delete_forward())
        if k == KEY_LEFT:
            self._move_left()
            return TextFieldKeyResult(True, False)
        if k == KEY_RIGHT:
        ...
        if k == KEY_DOWN:
            ...
            return TextFieldKeyResult(True, False)
        if k == KEY_PAGEUP:
            self._move_vertical(-self._page)
            return TextFieldKeyResult(True, False)
        if k == KEY_PAGEDOWN:
            self._move_vertical(self._page)
            return TextFieldKeyResult(True, False)
        ...
        if k == KEY_END:
            ...
            self.col = len(self.lines[self.row].as_bytes())
            return TextFieldKeyResult(True, False)
        # Paste. Multi-line clipboard content is inserted verbatim —
        # that's the whole point of a multi-line input, so unlike
        # ``TextField`` we don't sanitize newlines out.
        if clipboard_chord(event) == CLIP_PASTE:
            var pasted = clipboard_paste()
            if len(pasted.as_bytes()) == 0:
                return TextFieldKeyResult(True, False)
            return TextFieldKeyResult(True, self.insert(pasted))
        # An unbound Cmd/Ctrl chord must not fall through to the
        # printable branch (Cmd+B inserting ``b``) — same reasoning as
        # ``TextField.handle_key``.
        if chord:
            beep()
            return TextFieldKeyResult(True, False)
        if (event.mods & MOD_ALT) != 0:
    ...
    def handle_mouse(mut self, event: Event, rect: Rect) -> Bool:
        """Place the caret at the clicked cell. Returns True when the
        click landed inside ``rect``."""
        if event.kind != EVENT_MOUSE or event.button != MOUSE_BUTTON_LEFT:
            return False
        # Presses only: reacting to motion / release too would drag the
        # caret around under a moving pointer.
        if not event.pressed or event.motion:
            return False
        if not rect.contains(event.pos):
            return False
        self._has_goal = False
        self.row = self.scroll_row + (event.pos.y - rect.a.y)
        self._clamp_row()
        self._set_col_from_cell(self.scroll_col + (event.pos.x - rect.a.x))
        return True

    # --- paint --------------------------------------------------------

    def _ensure_visible(mut self, width: Int, height: Int):
        if height > 0:
            if self.row < self.scroll_row:
                self.scroll_row = self.row
            if self.row >= self.scroll_row + height:
                self.scroll_row = self.row - height + 1
            var max_scroll = len(self.lines) - height
            if max_scroll < 0:
                max_scroll = 0
            ...
            if self.scroll_row < 0:
                self.scroll_row = 0
        if width > 0:
            var cell = self.cursor_cell()
            if cell < self.scroll_col:
                self.scroll_col = cell
            # Keep one cell of room so the caret is drawn *inside* the
            # strip rather than on the clip boundary.
            if cell > self.scroll_col + width - 1:
                self.scroll_col = cell - width + 1
            if self.scroll_col < 0:
                self.scroll_col = 0

    def paint(mut self, mut canvas: Canvas, rect: Rect, focused: Bool):
        """Render the buffer into ``rect`` using the shared input-field
        colors, scrolling both axes to keep the caret visible. Takes
        ``mut self`` to update the scroll offsets and record the page
        height."""
        var attr = text_field_bg()
        var painter = Painter(rect)
        painter.fill(canvas, rect, String(" "), attr)
        var width = rect.b.x - rect.a.x
        var height = rect.b.y - rect.a.y
        if width <= 0 or height <= 0:
            return
        self._page = height
        self._ensure_visible(width, height)
        for i in range(height):
            var idx = self.scroll_row + i
            if idx >= len(self.lines):
                break
            var line = self.lines[idx]
            var start_byte = utf8_byte_of_cell(line, self.scroll_col)
            var lb = line.as_bytes()
            if start_byte >= len(lb):
                continue
            var visible = String(
                StringSpan(unsafe_from_utf8=lb[start_byte:len(lb)])
            )
            _ = painter.put_text(
                canvas, Point(rect.a.x, rect.a.y + i), visible, attr,
            )
        if focused:
            var cur_x = rect.a.x + self.cursor_cell() - self.scroll_col
            var cur_y = rect.a.y + self.row - self.scroll_row
            if cur_x >= rect.a.x and cur_x < rect.b.x \
                    and cur_y >= rect.a.y and cur_y < rect.b.y:
                # Reverse-video block, preserving any glyph underneath —
                # matches ``TextField.paint`` so both inputs show the
                # same caret under every theme.
                var glyph = canvas.get(cur_x, cur_y).glyph
                if len(glyph.as_bytes()) == 0:
                    glyph = String(" ")
                painter.set(
                    canvas, cur_x, cur_y,
                    Cell(glyph, Attr(attr.bg, attr.fg), 1),
